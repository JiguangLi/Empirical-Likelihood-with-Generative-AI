...
# -----------------------------------------------------------------------------
# Basis builder (no intercept): natural cubic regression spline w/ paper tilde
# -----------------------------------------------------------------------------
def make_basis_ns_patsy_paper(
    ...
    *,
    ...
    """Build spline basis matrix B(W) (no intercept column)."""
...
# -----------------------------------------------------------------------------
_MIXED_V2_WORKER: Optional["MixedConditionalGenELV2"] = None


def _init_worker_mixed_v2(init_kwargs: Dict[str, Any]):
    global _MIXED_V2_WORKER
    _MIXED_V2_WORKER = MixedConditionalGenELV2(**init_kwargs)


def _solve_one_mixed_v2(task):
    i, seed, method, options = task
    v = _MIXED_V2_WORKER.draw_dirichlet(seed)
    out = _MIXED_V2_WORKER.fit_per_v(v=v, method=method, options=options)
...
# Main class
# -----------------------------------------------------------------------------
class MixedConditionalGenELV2(GenELV2):
    """GenELV2 with a mixed unconditional + conditional moment system."""

    def __init__(
        self,
        X: np.ndarray,
        W: np.ndarray,
        g_uncond: Optional[Callable[[np.ndarray, np.ndarray], np.ndarray]],
        g_cond: Optional[Callable[[np.ndarray, np.ndarray], np.ndarray]],
        theta0: np.ndarray,
        *,
        g_uncond_jac: Optional[Callable[[np.ndarray, np.ndarray], np.ndarray]] = None,
        g_cond_jac: Optional[Callable[[np.ndarray, np.ndarray], np.ndarray]] = None,
        # basis controls (only used if conditional block is present)
        df_first: int = 3,
        df_rest: int = 3,
        center_basis: bool = False,
        scale_basis: bool = False,
        add_constant: bool = False,
        whiten_basis: bool = False,
        svd_tol: float = 1e-10,
        ...
        if g_uncond is None and g_cond is None:
            raise ValueError("At least one of g_uncond or g_cond must be provided.")
        ...
        if not (np.isfinite(X).all() and np.isfinite(W).all()):
            raise ValueError("X/W contain NaN/Inf.")

        self.W = W
        self._g_uncond_user = g_uncond
        self._g_cond_user = g_cond
        self._g_uncond_jac_user = g_uncond_jac
        self._g_cond_jac_user = g_cond_jac
        ...
        # ---- Probe dimensions + vectorization per block ----
        self._g_uncond_vectorized = True
        self._g_cond_vectorized = True

        def _probe(func, *, name: str) -> Tuple[int, bool]:
            if func is None:
                return 0, True
            try:
                G = np.asarray(func(X, theta0), float)
                if G.ndim == 1:
                    if G.shape[0] != N:
                        raise ValueError
                    G = G.reshape(N, 1)
                if G.shape[0] != N:
                    raise ValueError
                return int(G.shape[1]), True
            except Exception:
                rows = [np.asarray(func(X[i], theta0), float).reshape(-1) for i in range(N)]
                G = np.vstack(rows)
                if G.ndim == 1:
                    G = G.reshape(N, 1)
                if G.shape[0] != N:
                    raise ValueError(f"{name} per-row evaluation produced inconsistent shapes.")
                return int(G.shape[1]), False

        self.q_uncond, self._g_uncond_vectorized = _probe(self._g_uncond_user, name="g_uncond")
        self.q_cond, self._g_cond_vectorized = _probe(self._g_cond_user, name="g_cond")

        # ---- Build basis for conditional block (if present) ----
        self.B_basis = np.empty((N, 0), dtype=float)
        self.K_basis = 0
        self._basis_meta: Dict[str, Any] = {
            "has_conditional_block": bool(self._g_cond_user is not None and self.q_cond > 0),
            "q_uncond": int(self.q_uncond),
            "q_cond": int(self.q_cond),
        }

        if self._g_cond_user is not None and self.q_cond > 0:
            ...
            # total budget for all moments
            q_budget_total = int(np.floor(max_moment_ratio * N))
            q_avail_for_cond = q_budget_total - int(self.q_uncond)
            if q_avail_for_cond <= 0:
                raise ValueError(
                    "Moment budget exhausted by unconditional moments: "
                    f"q_uncond={self.q_uncond} >= floor(max_moment_ratio*n)={q_budget_total}."
                )

            K_total_budget = int(np.floor(q_avail_for_cond / max(1, self.q_cond)))
            if K_total_budget <= 0:
                raise ValueError(
                    "Not enough moment budget for conditional block: "
                    f"q_avail_for_cond={q_avail_for_cond}, q_cond={self.q_cond} -> K_total_budget={K_total_budget}."
                )
            ...
            if whiten_basis and B_for_svd.shape[1] > 0 and K_budget_nonconst > 0:
                ...
                if B_for_svd.shape[1] > 0 and K_budget_nonconst > 0:
            ...
            self._basis_meta = {
                ...
                **basis_meta,
                ...
                "q_expanded_cond": int(self.K_basis * self.q_cond),
                "q_total": int(self.q_uncond + self.K_basis * self.q_cond),
            }

        self._mixed_cfg = dict(
            df_first=int(df_first),
            df_rest=int(df_rest),
        ...
        # ---- Call parent init (it will call our overridden _G(theta0)) ----
        if self._g_uncond_user is not None:
            fallback_g = self._g_uncond_user
        elif self._g_cond_user is not None:
            fallback_g = self._g_cond_user
        else:
            fallback_g = lambda X_, th_: np.zeros((X_.shape[0], 1), dtype=float)
        ...
        # expanded systems: column scaling is often redundant
        if hasattr(self, "use_column_scaling"):
            self.use_column_scaling = False

        # Optional ETEL initialization in the mixed moment space
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _ensure_2d(G: np.ndarray, N: int) -> np.ndarray:
        G = np.asarray(G, float)
        if G.ndim == 1:
            if G.shape[0] != N:
                raise ValueError("moment function returned wrong length")
            return G.reshape(N, 1)
        if G.shape[0] != N:
            raise ValueError("moment function wrong first dimension")
        return G

    def _eval_g_uncond(self, theta: np.ndarray) -> np.ndarray:
        N = self.X.shape[0]
        if self._g_uncond_user is None or self.q_uncond == 0:
            return np.empty((N, 0), dtype=float)
        if self._g_uncond_vectorized:
            return self._ensure_2d(self._g_uncond_user(self.X, theta), N)
        rows = [np.asarray(self._g_uncond_user(self.X[i], theta), float).reshape(-1) for i in range(N)]
        Gu = np.vstack(rows)
        if Gu.ndim == 1:
            Gu = Gu.reshape(N, 1)
        return Gu

    def _eval_g_cond(self, theta: np.ndarray) -> np.ndarray:
        N = self.X.shape[0]
        if self._g_cond_user is None or self.q_cond == 0:
            return np.empty((N, 0), dtype=float)
        if self._g_cond_vectorized:
            return self._ensure_2d(self._g_cond_user(self.X, theta), N)
        rows = [np.asarray(self._g_cond_user(self.X[i], theta), float).reshape(-1) for i in range(N)]
        Gc = np.vstack(rows)
        if Gc.ndim == 1:
            Gc = Gc.reshape(N, 1)
        return Gc
    ...
    def _G(self, theta: np.ndarray) -> np.ndarray:
        ...
        Gu = self._eval_g_uncond(theta)
        q_u = int(Gu.shape[1])

        if self._g_cond_user is None or self.q_cond == 0 or self.K_basis == 0:
            return Gu

        Gc0 = self._eval_g_cond(theta)
        q_c = int(Gc0.shape[1])
        B = self.B_basis
        K = int(B.shape[1])

        out = np.empty((N, q_u + K * q_c), dtype=float)
        if q_u > 0:
            out[:, :q_u] = Gu

        off = q_u
        for k in range(K):
            out[:, off:off + q_c] = B[:, [k]] * Gc0
            off += q_c
        return out
    ...
    # Jacobians (moments)
    # ------------------------------------------------------------------
    def _eval_g_uncond_jac(self, theta: np.ndarray) -> np.ndarray:
        """Return Ju: (N, q_uncond, d_theta)."""
        theta = np.asarray(theta, float).reshape(-1)
        N = self.X.shape[0]
        q_u = int(self.q_uncond)
        if q_u == 0:
            return np.empty((N, 0, self.d_theta), dtype=float)

        if self._g_uncond_jac_user is not None:
            Ju = np.asarray(self._g_uncond_jac_user(self.X, theta), float)
            if Ju.ndim != 3 or Ju.shape != (N, q_u, self.d_theta):
                raise ValueError(
                    f"g_uncond_jac returned shape {Ju.shape}, expected ({N}, {q_u}, {self.d_theta})"
                )
            return Ju

        eps = float(self._outer_fd_eps_local)
        if eps <= 0:
            raise ValueError("outer_fd_eps must be positive when g_uncond_jac is None")

        base = self._eval_g_uncond(theta)  # (N, q_u)
        Ju = np.empty((N, q_u, self.d_theta), dtype=float)
        for j in range(self.d_theta):
            th_p = theta.copy(); th_p[j] += eps
            th_m = theta.copy(); th_m[j] -= eps
            Gp = self._eval_g_uncond(th_p)
            Gm = self._eval_g_uncond(th_m)
            Ju[:, :, j] = (Gp - Gm) / (2.0 * eps)
        return Ju

    def _eval_g_cond_jac(self, theta: np.ndarray) -> np.ndarray:
        """Return Jc0: (N, q_cond, d_theta)."""
        theta = np.asarray(theta, float).reshape(-1)
        N = self.X.shape[0]
        q_c = int(self.q_cond)
        if q_c == 0:
        ...
        if self._g_cond_jac_user is not None:
            Jc = np.asarray(self._g_cond_jac_user(self.X, theta), float)
            if Jc.ndim != 3 or Jc.shape != (N, q_c, self.d_theta):
                raise ValueError(
                    f"g_cond_jac returned shape {Jc.shape}, expected ({N}, {q_c}, {self.d_theta})"
                )
            return Jc

        eps = float(self._outer_fd_eps_local)
        if eps <= 0:
            raise ValueError("outer_fd_eps must be positive when g_cond_jac is None")

        base = self._eval_g_cond(theta)  # (N, q_c)
        Jc = np.empty((N, q_c, self.d_theta), dtype=float)
        for j in range(self.d_theta):
            th_p = theta.copy(); th_p[j] += eps
            th_m = theta.copy(); th_m[j] -= eps
            Gp = self._eval_g_cond(th_p)
            Gm = self._eval_g_cond(th_m)
            Jc[:, :, j] = (Gp - Gm) / (2.0 * eps)
        return Jc

    def _G_jac(self, theta: np.ndarray) -> np.ndarray:
        """Jacobian of mixed moment matrix: (N, q_total, d_theta)."""
        theta = np.asarray(theta, float).reshape(-1)
        N = self.X.shape[0]
        ...
        q_total = q_u + (K * q_c if (q_c > 0 and K > 0) else 0)
        J = np.empty((N, q_total, self.d_theta), dtype=float)

        off = 0
        if q_u > 0:
            Ju = self._eval_g_uncond_jac(theta)
            J[:, off:off + q_u, :] = Ju
            off += q_u

        if q_c > 0 and K > 0:
            Jc0 = self._eval_g_cond_jac(theta)  # (N, q_c, d)
            B = self.B_basis
            for k in range(K):
                J[:, off:off + q_c, :] = B[:, [k], None] * Jc0
                off += q_c

        return J

    def _g_jac_mixed(self, X: np.ndarray, theta: np.ndarray) -> np.ndarray:
        _ = X
        return self._G_jac(theta)

    # ------------------------------------------------------------------
    # Parallel bootstrap (workers must rebuild the basis)
    # ------------------------------------------------------------------
    def fit_in_parallel(
        self,
        ...
        init_kwargs = dict(
            X=self.X,
            W=self.W,
            g_uncond=self._g_uncond_user,
            g_cond=self._g_cond_user,
            g_uncond_jac=self._g_uncond_jac_user,
            g_cond_jac=self._g_cond_jac_user,
            theta0=self.theta0,
            # basis
        ...
        with ProcessPoolExecutor(
            ...
            initializer=_init_worker_mixed_v2,
            initargs=(init_kwargs,),
        ) as ex:
            tasks = ((i, seeds[i], method, opts) for i in range(self.B))
            for i, out in tqdm(
                ex.map(_solve_one_mixed_v2, tasks, chunksize=chunksize),
                total=self.B,
                desc="Bootstrap (mixed)",
                unit="draw",
            ):
                thetas[i] = out["theta"]
                losses[i] = out["loss"]
